package com.apm.plugin.string.service;

import com.alibaba.fastjson.JSON;
import com.lcl.sw.demosw2.model.User;
...
public class SwService1Interceptor implements InstanceMethodsAroundInterceptor {
    private static final ILog LOGGER = LogManager.getLogger(SwService1Interceptor.class);
    ...
    public void beforeMethod(EnhancedInstance enhancedInstance, Method method, Object[] objects, Class<?>[] classes, MethodInterceptResult methodInterceptResult) throws Throwable {
        ...
        AbstractSpan span = ContextManager.createLocalSpan("SW/sw2/user");
        /*
    ...
    public Object afterMethod(EnhancedInstance enhancedInstance, Method method, Object[] objects, Class<?>[] classes, Object o) throws Throwable {
        ...
        LOGGER.debug("afterMethod: " + JSON.toJSONString(ret));
        if("laicunli".equals(ret.getCode())){
            User user = new User();
            user.setCode("王大锤");
            user.setEmail(ret.getEmail());
            user.setCreateBy(ret.getCreateBy());
            user.setCreateTime(ret.getCreateTime());
            user.setPassword(ret.getPassword());
            user.setNickName(ret.getNickName());
            user.setRole(ret.getRole());
            user.setStatus(ret.getStatus());
            user.setUpdateBy(ret.getUpdateBy());
            user.setUpdateTime(ret.getUpdateTime());
            return user;
        }
        return ret;
    }
